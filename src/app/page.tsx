  ...
  return (
    <section className="flex min-h-dvh flex-col items-center justify-center gap-5 px-5 py-14 text-center lg:gap-6 lg:px-14">
      <h1 className="font-accent max-w-[300px] text-[32px] leading-[1.2] font-bold text-[#211C2E] sm:max-w-[440px] sm:text-[40px] lg:max-w-[780px] lg:text-[52px] lg:leading-[1.18]">
        <span className="text-brand-accent">Yasno</span> перетворює хаос думок
        на план дня.
      </h1>
      ...
      <p className="font-body max-w-[300px] text-[15px] leading-relaxed font-medium text-[#746C82] sm:max-w-[420px] sm:text-[16px] lg:max-w-[480px] lg:text-[18px] lg:leading-[1.6]">
        Скажи чи напиши все, що крутиться в голові — застосунок сам перетворить
        це на задачі з часом, пріоритетом і датою.
      </p>

      <Link
        href="/login"
        className="font-accent bg-brand-accent mt-1 rounded-full px-8 py-4 text-[14px] font-bold text-white shadow-[0_10px_26px_rgba(255,107,71,0.35)] transition-transform hover:-translate-y-0.5 active:translate-y-0 lg:px-9 lg:py-[15px] lg:text-[15px]"
      >
        Почати →
      </Link>

      <p className="font-body text-[11.5px] text-[#746C82] opacity-75">
        Без картки. 30 секунд на реєстрацію.
      </p>
    </section>
  );
}
    ...
    <section className="flex flex-col items-center gap-8 border-t border-[rgba(33,28,46,0.08)] px-5 py-16 lg:gap-9 lg:px-14 lg:py-24">
      <div className="flex flex-col items-center gap-2 text-center">
        ...
        <p className="font-body max-w-[300px] text-[13.5px] text-[#746C82] sm:max-w-[420px] lg:text-[15px]">
          Інтерфейс тихо змінюється разом із часом доби — не просто темна тема,
          а справжня атмосфера.
        </p>
      </div>
...
const HOW_IT_WORKS_STEPS: HowItWorksStep[] = [
  {
    ...
    description:
      "Текстом чи голосом — без структури, без форматування, просто потік думок.",
    icon: (
      <svg
        viewBox="0 0 24 24"
        fill="none"
  ...
  },
  {
    ...
    description:
      "Час, тривалість, пріоритет — визначаються самі, ти лише підтверджуєш.",
    icon: (
      <svg
        viewBox="0 0 24 24"
        fill="none"
        ...
        {HOW_IT_WORKS_STEPS.map((step, index) => (
          <Fragment key={step.title}>
            <div className="flex w-[240px] flex-col items-center gap-3 text-center lg:max-w-[220px]">
              <div className="text-brand-accent h-12 w-12 lg:h-14 lg:w-14">
                {step.icon}
              </div>
              <h3 className="font-body text-[14px] font-bold text-[#211C2E] lg:text-[15px]">
                {step.title}
              </h3>
            ...
            {index < HOW_IT_WORKS_STEPS.length - 1 && (
              <span
                aria-hidden="true"
                className="text-[20px] text-[#211C2E]/20"
              >
...
function LandingFooter() {
  return (
    <footer className="flex flex-col items-center gap-4 border-t border-[rgba(33,28,46,0.08)] px-5 py-12 text-center lg:gap-5 lg:py-16">
      <Link
        href="/login"
        className="font-accent bg-brand-accent rounded-full px-7 py-3.5 text-[13px] font-bold text-white shadow-[0_10px_26px_rgba(255,107,71,0.35)] transition-transform hover:-translate-y-0.5 active:translate-y-0 lg:text-[14px]"
      >
        Почати →
      </Link>
